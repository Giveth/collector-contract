...
describe('unit/Collector', () => {
  ...
  describe('#changeBeneficiary', () => {
    ...
    before(() => {
      ...
      newBeneficiary = actors.other().address;
    });

    describe('works and', () => {
      it('emits the beneficiary changed event', async () => {
        const oldBeneficiary = await context.collector.beneficiary();
        await expect(subject(actors.owner(), newBeneficiary))
          .to.emit(context.collector, 'BeneficiaryChanged')
          .withArgs(oldBeneficiary, newBeneficiary);
      });

      it('changes the beneficiary address', async () => {
        await subject(actors.owner(), actors.other());
        expect(await context.collector.beneficiary()).to.be.eq(newBeneficiary);
      });
    });
